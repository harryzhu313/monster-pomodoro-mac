...
**本机网络隐私：** `iterations/` 下的任何文档（包括 PRD、CONTENT、`.plan/`、验收证据和报告）不得记录用户本机的网络环境信息，包括但不限于 Surge 等网络工具及其配置或状态、Wi-Fi/SSID、真实的本机/局域网/公网 IP 地址、代理节点、代理端口、路由规则、网关、DNS 和网络接口信息。排障确有需要时，只在当前对话中临时使用；写入迭代文档时必须删除或概括为不含本机网络细节的结论。通用的 `localhost`、回环地址和本地测试端口可以保留；产品技术参数、线上基础设施配置以及“代理商”等业务内容不受此规则限制。
